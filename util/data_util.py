...
def collation_fn_voxelmean_tta(batch_list):
    ...
    return samples
'''Xavier Use'''
def collation_fn_voxelmean_tta_test(batch_list):
    """
    :param batch:
    :return:   coords_batch: N x 4 (x,y,z,batch)
    ...
    for batch in batch_list:
        coords, xyz, feats, labels, inds_recons,file_path = list(zip(*batch))
        inds_recons = list(inds_recons)
        ...
        sample = (coords, xyz, feats, labels, offset, inds_recons,file_path)
        samples.append(sample)
